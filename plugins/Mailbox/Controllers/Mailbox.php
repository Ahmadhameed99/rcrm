...
class Mailbox extends Security_Controller {
    ...
    function index($mailbox_id = 0, $client_id = 0) {
    ...
    function listData($mode = "inbox", $mailbox_id = 0, $client_id = 0) {
        $this->can_access_this_mailbox($mailbox_id);
        ...
        $list_data = $this->Mailbox_emails_model->get_details($options)->getResult();
        $result = array();
    ...
    function send() {
        ...
        $custom_bcc = $this->request->getPost('email_bcc');
        $subject = $this->request->getPost('subject');
        $message = decode_ajax_post_data($this->request->getPost('message'));

        $default_bcc = $mailbox_info->send_bcc_to; //get default settings
        $bcc_emails = "";

        if ($default_bcc && $custom_bcc && $default_bcc !== $custom_bcc) {
        ...
        if ($save_as_draft) {
            ...
            if (!send_app_mail($to_array, $subject, $message, array("attachments" => $attachments, "cc" => $cc_array, "bcc" => $bcc_emails, "reply_to" => $mailbox_info->imap_email))) {
    ...
    private function prepare_emails_array($emails_data = "") {
        ...
        foreach ($emails as $email_value) {
            if (is_numeric($email_value)) {
                //selected a contact
                array_push($emails_array, $this->Users_model->get_one($email_value)->email);
            } else {
                //inputted an email address
                array_push($emails_array, $email_value);
            }
        }
    ...
    function clientEmails($client_id) {
        return $this->index(0, $client_id);
    }
