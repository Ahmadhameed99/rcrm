...
app_hooks()->add_filter('app_filter_admin_settings_menu', function ($settings_menu) {
    $settings_menu["setup"][] = array("name" => "mailbox", "url" => "mailbox_settings");
...
if (!function_exists('mailbox_details_view_ajax_tab')) {
    ...
    function mailbox_details_view_ajax_tab($client_id = 0) {
        if (!$client_id) {
            return;
        }
...
app_hooks()->add_filter('app_filter_action_links_of_Mailbox', function ($action_links_array) {
    ...
    $action_links_array[] = anchor("https://rise.classiccompiler.com/documentation/mailbox/", "Documentation", array("target" => "_blank"));
