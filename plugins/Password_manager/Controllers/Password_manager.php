...
class Password_manager extends \App\Controllers\Security_Controller {
    ...
    function software_license_modal_form() {
        ...
        // $model_info->license_key = decode_id($license_key, "license_key"); Decrypt by me
        $model_info->license_key = base64_decode($license_key);
    ...
    function save_software_license() {
        ...
        // $license_key = encode_id($this->request->getPost('license_key'), "license_key");   Encrypt by me
        $license_key = base64_encode($this->request->getPost('license_key'));
        $client_id = $this->request->getPost('client_id');
        $created_by_client = $this->request->getPost('created_by_client');
    ...
    private function _make_software_license_row($data) {
        ...
        //only creator can edit/delete passwords
        $actions = modal_anchor(get_uri("password_manager/view_software_license"), app_lang('preview_btn'), array("class" => "border-0 rounded p-1 bg-primary text-light", "title" => app_lang('password_manager_password_details'), "data-modal-title" => app_lang('password_manager_software_license') . " #$data->id", "data-post-id" => encode_id($data->id, "software_license_password_id")));
        if ($data->created_by == $this->login_user->id || ($data->created_by_client == 1 && $this->login_user->is_admin)) {
